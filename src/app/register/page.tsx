...
interface UserFormInput {
  ...
  agree: boolean;
   ...
   function Register(){
    ...
    const { register, handleSubmit, watch, formState: { errors } } = useForm<UserFormInput>({
      mode: "onChange"
    }); 
    ...
    const isButtonDisabled = Object.values(watchField).some(value => value === '' || value === false);
    ...
    return <>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex-1 max-md:hidden justify-center flex items-center" style={{backgroundImage: `url(${bglt.src}), url(${bglb.src}), url(${bgrt.src}), url(${bgrb.src})`}}>
             <Image src={img} alt="image" className="w-[50%] min-h-[50%] ml-[40px]" />
          </div>
         ...
         <div className="flex flex-col justify-center items-center">
          <div className="flex flex-col text-[13px]">
              ...
              <div className="flex items-center border-2 rounded-lg hover:border-gray-400 min-h-[50px] w-[470px] max-sm:max-w-[395px]">
              <input type="text" {...register("first_name", {required: true, onChange: (e) => setFirstName(e.target.value), })}
          ...
          {errors.first_name && <p className="text-red-600 text-[13px] font-medium mt-1">This field is required.</p>}
          </div>

          <div className="flex flex-col text-[13px]">
              <label className="mt-3 text-base font-medium">Last Name</label>
              <div className="flex items-center border-2 rounded-lg hover:border-gray-400 min-h-[50px] w-[470px] max-sm:max-w-[395px]">
              <input type="text" {...register("last_name", { required: true, onChange: (e) => setLastName(e.target.value), })}
               className="flex-grow pl-3 pr-3 rounded-lg outline-none" placeholder=""/>
              {errors.last_name ? ( 
                 <FontAwesomeIcon icon={faXmarkCircle} className=" text-red-600 mr-5 size-4" />
              ) : lastName && (
          ...
          <div className="flex flex-col text-[13px]">
              <label className="mt-3 text-base font-medium">Email <span className="text-gray-800">*</span></label>
              <div className="flex items-center border-2 rounded-lg hover:border-gray-400 min-h-[50px] w-[470px] max-sm:max-w-[395px]">
              <input type="email" {...register("email", { required: true, pattern: {value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,message: "Invalid email address"}, onChange: (e) => setEmail(e.target.value)})}
                className="flex-grow pl-3 pr-3 rounded-lg outline-none" placeholder=""/>
                {errors.email ?  ( 
                  <FontAwesomeIcon icon={faXmarkCircle} className=" text-red-600 mr-5 size-4" />
              ) : email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/) && (
          ...
          <div className="flex flex-col text-[13px]">
              <label className="mt-3 text-base font-medium">Create Password <span className="text-gray-800">*</span></label>
              <div className="flex items-center border-2 rounded-lg hover:border-gray-400 min-h-[50px] w-[470px] max-sm:max-w-[395px]">
               <input type={createPasswordView ? "text" : "password"} {...register("create_password")}
                className="flex-grow pl-3 pr-3 rounded-lg outline-none " placeholder="Enter your password"/>
               <FontAwesomeIcon icon={createPasswordView ? faEyeSlash :faEye} className=" text-[#717171] mr-5" onClick={createPasswordViewVisible} />
              </div>           
          </div>
          <div className="flex flex-col text-[13px]">
              <label className="mt-3 text-base font-medium">Confirm Password <span className="text-gray-800">*</span></label>
              <div className="flex items-center border-2 rounded-lg hover:border-gray-400 min-h-[50px] w-[470px] max-sm:max-w-[395px]">
               <input type={confirmPasswordView ? "text" : "password"} {...register("confirm_password")}
                className ="flex-grow pl-3 pr-3 rounded-lg outline-none " placeholder="Enter your password"/>
          ...
          <div className="flex flex-row gap-[30px] max-sm:mr-auto text-gray-500 items-center mt-5 mr-[7.5rem]">
             <h1 className="text-[16px] font-semibold text-gray-800">Gender <span className="text-gray-800">*</span></h1>
          <div className="flex items-center gap-2">
              <input type="radio" value="Femal" {...register('gender', { required: true })} className="min-h-8 border-2 rounded-lg" />
              <label className="text-base font-medium text-[14px]">Female</label>
          </div>
          <div className="flex items-center gap-2">
              <input type="radio" value="Male" {...register('gender', { required: true })} className="min-h-8 border-2 rounded-lg" />
              <label className="text-base font-medium text-[14px]">Male</label>
          </div>
          <div className="flex items-center gap-2">
              <input type="radio" value="Other" {...register('gender', { required: true })} className="min-h-8 border-2 rounded-lg" />
          ...
          <div className="flex max-sm:mr-auto flex-row font-medium text-gray-500 gap-[10px] items-center mt-2 mr-[11rem]">
              <input type="checkbox" {...register('agree', { required: true })} className="min-h-10 w-5 border-2" />
          ...
          <div className="flex justify-center mt-5">
              <Button type="submit" label="Create New Account" disabled={isButtonDisabled} />
          </div>
